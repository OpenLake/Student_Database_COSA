...
import {
  Search, Plus, Eye, Pencil, Trash2, Copy, Archive,
  ChevronDown, MoreHorizontal, FileText, LayoutGrid, List,
  X, Filter, SquareUser,
  Info
} from "lucide-react";
...
export default function Templates() {
  ...
  useEffect(()=>{
    ...
    async function getTemplates(){
      /* ── sample data ─────────────────────────────────────────── */
      const TEMPLATES = [
        { id: 1, name: "Academic Excellence Award",   category: "Academic",     status: "Active",   createdBy: "Priya Sharma",   modified: "2026-02-18", color: "#4A7C59" },
        { id: 2, name: "Event Participation",          category: "Events",       status: "Active",   createdBy: "John Matthews",  modified: "2026-02-15", color: "#7C4A6B" },
        { id: 3, name: "Workshop Completion",          category: "Workshop",     status: "Draft",    createdBy: "Arjun Verma",    modified: "2026-02-10", color: "#7C6B4A" },
        { id: 4, name: "Sports Achievement",           category: "Sports",       status: "Active",   createdBy: "Meera Nair",     modified: "2026-01-30", color: "#4A6B7C" },
        { id: 5, name: "Hackathon Winner",             category: "Events",       status: "Draft",    createdBy: "Rahul Singh",    modified: "2026-01-25", color: "#7C4A4A" },
        { id: 6, name: "Research Contribution",        category: "Academic",     status: "Archived", createdBy: "Priya Sharma",   modified: "2025-12-20", color: "#5A7C4A" },
        { id: 7, name: "Leadership Certificate",       category: "Workshop",     status: "Active",   createdBy: "John Matthews",  modified: "2026-02-01", color: "#4A4A7C" },
        { id: 8, name: "Internship Completion",        category: "Academic",     status: "Draft",    createdBy: "Arjun Verma",    modified: "2026-02-12", color: "#7C7C4A" },
      ];
  ...
  const filters = useMemo(()=>({
    ...
    creators: [...new Set(templates?.map(t => t.createdBy))]

  }),[templates])

  const duplicate = (id) => {
    const t = templates.find(t => t.id === id);
    setTemplates(ts => [...ts, { ...t, id: Date.now(), name: `${t.name} (Copy)`, status: "Draft" }]);
  };
  const archive = id => setTemplates(ts => ts.map(t => t.id === id ? { ...t, status: "Archived" } : t));
  const del = (id) => setTemplates(ts => ts.filter(t => t.id !== id));

  const hasFilters = catFilter !== "All" || statusFilter !== "All" || creatorFilter !== "All" || search;
  const clearAll   = () => { setCat("All"); setStatus("All"); setCreator("All"); setSearch(""); };
    ...
    <div className="min-h-screen bg-[#FEFCE8] p-6">
      ...
      <div className="flex items-center gap-2.5 mb-5 flex-wrap">
        ...
        <div className="flex items-center bg-white border border-yellow-100 rounded-xl p-1 gap-0.5">
          <button onClick={() => setView("grid")}
            ...
            style={view === "grid" ? { background: "linear-gradient(135deg, #2A6040, #1E4A30)" } : {}}
          >
            <LayoutGrid size={15} />
          </button>
          <button onClick={() => setView("list")}
            className={`p-1.5 !rounded-lg transition-colors ${view === "list" ? "text-white shadow-sm" : "text-gray-400 hover:text-gray-600"}`}
            style={view === "list" ? { background: "linear-gradient(135deg, #2A6040, #1E4A30)" } : {}}
          >
            <List size={15} />
          </button>
        </div>
        ...
        {hasFilters && (
          <button onClick={clearAll} className="text-xs font-semibold bg-white text-yellow-700 !rounded-2xl border hover:text-yellow-900 px-2 py-2 ">
            Clear all
          </button>
        )}
      ...
      {filtered?.length === 0 && (
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          {search 
            ? <p className="text-xs font-bold text-gray-400">Try adjusting your filters or search</p>
            : <p className="text-xs font-bold text-gray-400">No templates exist. Create one today</p>
          }
            ...
            >
              ...
              <div className="relative h-36 w-full border-b border-yellow-50">
                ...
                <div className="absolute bottom-2.5 right-2.5">
                  <span className={`inline-flex items-center mr-3 mb-1 gap-1 text-[9px] font-bold px-2 py-0.5 rounded-full ${STATUS_STYLE[t.status]}`}>
                    <span className={`w-1 h-1 rounded-full ${STATUS_DOT[t.status]}`} />
                    {t.status}
                  </span>
                </div>
              </div>
                ...
                <div className="rounded-xl p-2.5 space-y-2 ">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      {(userRole.toUpperCase() === "PRESIDENT" || userRole.toUpperCase().startsWith("GENSEC") || t.name.includes("Copy"))
                        ?(
                          <>
                            <button
      ...
      {view === "list" && filtered?.length > 0 && (
        <div className="bg-white border border-yellow-100 rounded-2xl overflow-hidden shadow-sm">
          <table className="w-full border-collapse">
            <thead>
              <tr style={{ background: "linear-gradient(135deg, #2A6040, #1E4A30)" }}>
                {["Template", "Category", "Status", "Created By", "Last Modified", "Actions"].map((h, i, arr) => (
                  <th key={h}
            ...
            <tbody>
              {filtered.map((t, i) => (
                <tr key={t.id}
                  ...
                  style={{ borderBottom: i < filtered.length - 1 ? "1px solid #fef9c3" : "none" }}
                >
                  <td className="px-5 py-1.5" style={{ borderRight: "1px solid #fef3c7" }}>
                    <div className="flex items-center">
                      <p className="text-sm font-bold text-gray-900">{t.name}</p>
                    </div>
                  </td>
                  <td className="px-4 py-1.5" style={{ borderRight: "1px solid #fef3c7" }}>
                  ...
                  <td className="px-4 py-1.5" style={{ borderRight: "1px solid #fef3c7" }}>
                    <span className={`inline-flex items-center gap-1.5 text-[10px] font-semibold px-2.5 py-0.5 rounded-full ${STATUS_STYLE[t.status]}`}>
                  ...
                  <td className="px-4 py-1.5" style={{ borderRight: "1px solid #fef3c7" }}>
                    <div className="flex items-center">
                      <span className="text-xs text-gray-600 font-medium">{t.createdBy}</span>
                    </div>
                  </td>
                  <td className="px-4 py-1.5" style={{ borderRight: "1px solid #fef3c7" }}>
                    <span className="text-xs text-gray-500 font-medium">{t.modified}</span>
                  </td>
                  <td className="px-4 py-1.5">
                    <div className="flex items-center gap-1">
                      ...
                      <button className="p-1.5 !rounded-lg text-amber-400 border hover:bg-gray-50 transition-colors" onClick={() => duplicate(t.id)}>
                        <Copy size={14} />
                      </button>
                      ...
                      ? (
                        <>
                          ...
                          <button className="p-1.5 !rounded-lg text-red-400 border hover:bg-red-50 hover:text-red-500 transition-colors" onClick={() => del(t.id)}>
                            <Trash2 size={14} />
                          </button>
                        </>
                      )
