...
export default function GenSecCultPage() {
  ...
  const fetchSkills = async () => {
    try {
      setLoading(true);
      const res = await axios.get(`${backendUrl}/skills/unendorsed/cultural`);
      setSkills(res.data);
      setError(null);
    } catch (err) {
  ...
  const endorseSkill = async (skillId) => {
    try {
      await axios.put(`${backendUrl}/skills/endorse-cultural/${skillId}`);
    ...
    <div className="max-w-4xl mx-auto p-6 bg-white shadow-lg rounded-lg">
      <header className="mb-8">
        <h1 className="text-2xl md:text-3xl font-bold text-gray-800">
          Cultural Skills Pending Endorsement
        </h1>
        <p className="text-gray-600 mt-2">
          Review and endorse cultural skills submitted by students
        </p>
      </header>
      ...
      {loading ? (
        ...
        <div className="bg-gray-50 p-8 rounded-lg border border-gray-200 text-center">
          <p className="text-gray-600 font-medium">No unendorsed cultural skills found</p>
          ...
          {skills.map((skill) => (
            ...
            >
              <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
                ...
                <button
                  className="px-5 py-2 bg-purple-600 text-white font-medium rounded-lg hover:bg-purple-700 focus:ring-2 focus:ring-purple-500 focus:ring-opacity-50 transition-colors shadow-sm whitespace-nowrap"
