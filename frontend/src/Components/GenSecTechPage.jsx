...
export default function GenSecTechPage() {
  ...
  const fetchSkills = async () => {
    try {
      setLoading(true);
      const res = await axios.get(`${backendUrl}/skills/unendorsed/tech`);
      setSkills(res.data);
      setError(null);
    } catch (err) {
  ...
  const endorseSkill = async (skillId) => {
    try {
      await axios.put(`${backendUrl}/skills/endorse/${skillId}`);
    ...
    <div className="max-w-4xl mx-auto p-6 bg-white shadow-lg rounded-lg">
      ...
      {loading ? (
        ...
        <div className="bg-gray-50 p-8 rounded-lg border border-gray-200 text-center">
          <p className="text-gray-600 font-medium">No unendorsed tech skills found</p>
