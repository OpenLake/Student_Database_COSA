...
export function ShowSkills() {
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6 max-w-2xl mx-auto mt-8">
      <h2 className="text-xl font-semibold mb-4 text-gray-800">Student Skills</h2>
      ...
      {skills.length > 0 && (
        <div className="mt-6">
          ...
          <ul className="bg-gray-50 rounded-md border border-gray-200 divide-y divide-gray-200">
            {skills.map((skill) => (
              <li key={skill._id} className="p-3 flex items-center justify-between hover:bg-gray-100">
                <div>
                  <span className="font-medium">{skill.skillName}</span>
                  <span className="text-sm text-gray-500 ml-2">({skill.skillType})</span>
                </div>
                <span className={`px-2 py-1 rounded-full text-xs font-medium ${skill.endorsed ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-800"}`}>
      ...
      {skills.length === 0 && !error && !loading && id && (
        <p className="mt-4 text-gray-500 text-center py-4">No skills found for this student ID</p>
      )}
    </div>
  );
}
...
export function AddSkill() {
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6 max-w-2xl mx-auto mt-8">
      <h2 className="text-xl font-semibold mb-4 text-gray-800">Add New Skill</h2>
