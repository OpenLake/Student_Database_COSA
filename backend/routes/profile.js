...
router.put("/updateStudentProfile", isAuthenticated, async (req, res) => {
  try {
    ...
    if (updatedDetails.personal_info) {
      ...
      if (date_of_birth) {
        user.personal_info.date_of_birth = date_of_birth;
      }
      if (profilePic) {
    ...
    if (updatedDetails.academic_info) {
      ...
      if (program) {
        user.academic_info.program = program;
      }
      if (branch) {
        user.academic_info.branch = branch;
      }
      if (batch_year) {
        user.academic_info.batch_year = batch_year;
      }
      if (current_year) {
        user.academic_info.current_year = current_year;
      }
      if (cgpa !== undefined) {
        user.academic_info.cgpa = cgpa;
      }
    }
    ...
    if (updatedDetails.contact_info) {
      ...
      if (hostel) {
        user.contact_info.hostel = hostel;
      }
      if (room_number) {
        user.contact_info.room_number = room_number;
      }
