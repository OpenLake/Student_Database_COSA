...
const seedOrganizationalUnits = async () => {
  ...
  const testPresidentUnit = new OrganizationalUnit({
    ...
    category: "independent",
    contact_info: {
      email: "test_president_gymkhana@iitbhilai.ac.in",
      social_media: [],
    },
  });
  ...
  const councils = await OrganizationalUnit.find({
    type: "Council",
    unit_id: { $not: /_TEST/ },
  });
  ...
  const testCouncilsData = [
    {
      unit_id: "COUNCIL_CULTURAL_TEST",
      name: "Test Cultural Council",
      type: "Council",
      description: "Test council for cultural activities.",
      hierarchy_level: 1,
      category: "cultural",
      contact_info: {
        email: "test_gensec_cult@iitbhilai.ac.in",
        social_media: [],
      },
      parent_unit_id: presidentUnit._id,
    },
    {
      unit_id: "COUNCIL_SCITECH_TEST",
      name: "Test SciTech Council",
      type: "Council",
      description: "Test council for scitech activities.",
      hierarchy_level: 1,
      category: "scitech",
      contact_info: {
        email: "test_gensec_scitech@iitbhilai.ac.in",
        social_media: [],
      },
      parent_unit_id: presidentUnit._id,
    },
    {
      unit_id: "COUNCIL_SPORTS_TEST",
      name: "Test Sports Council",
      type: "Council",
      description: "Test council for sports activities.",
      hierarchy_level: 1,
      category: "sports",
      contact_info: {
        email: "test_gensec_sports@iitbhilai.ac.in",
        social_media: [],
      },
      parent_unit_id: presidentUnit._id,
    },
    {
      unit_id: "COUNCIL_ACAD_TEST",
      name: "Test Academic Council",
      type: "Council",
      description: "Test council for academic activities.",
      hierarchy_level: 1,
      category: "academic",
      contact_info: {
        email: "test_gensec_acad@iitbhilai.ac.in",
        social_media: [],
      },
      parent_unit_id: presidentUnit._id,
    },
  ...
  const testClubsData = [
    {
      unit_id: "CLUB_CULTURAL_TEST",
      name: "Test Cultural Club",
      type: "Club",
      description: "A test club for cultural events.",
      hierarchy_level: 2,
      category: "cultural",
      contact_info: {
        email: "test_cultural_club@iitbhilai.ac.in",
        social_media: [],
      },
      parent_unit_id: testCouncilMap.cultural,
    },
    {
      unit_id: "CLUB_SCITECH_TEST",
      name: "Test SciTech Club",
      type: "Club",
      description: "A test club for scitech events.",
      hierarchy_level: 2,
      category: "scitech",
      contact_info: {
        email: "test_scitech_club@iitbhilai.ac.in",
        social_media: [],
      },
      parent_unit_id: testCouncilMap.scitech,
    },
    {
      unit_id: "CLUB_SPORTS_TEST",
      name: "Test Sports Club",
      type: "Club",
      description: "A test club for sports events.",
      hierarchy_level: 2,
      category: "sports",
      contact_info: {
        email: "test_sports_club@iitbhilai.ac.in",
        social_media: [],
      },
      parent_unit_id: testCouncilMap.sports,
    },
    {
      unit_id: "CLUB_ACAD_TEST",
      name: "Test Academic Club",
      type: "Club",
      description: "A test club for academic events.",
      hierarchy_level: 2,
      category: "academic",
      contact_info: {
        email: "test_acad_club@iitbhilai.ac.in",
        social_media: [],
      },
      parent_unit_id: testCouncilMap.academic,
    },
  ];
...
const seedUsers = async () => {
  ...
  console.log(
    `Created and registered ${localAuthUsers.length} local auth users.`,
  );
...
const seedPositions = async () => {
  ...
  if (positionsToCreate.length > 0) {
    ...
    console.log(
      `Created ${positionsToCreate.length} positions for test units.`,
    );
  } else {
...
const seedPositionHolders = async () => {
  ...
  const testClubs = await OrganizationalUnit.find({
    type: "Club",
    name: /Test/,
  });
  ...
  for (const club of testClubs) {
    ...
    const clubPositions = testPositions.filter((p) =>
      p.unit_id._id.equals(club._id),
    );
    ...
    if (coreMemberPos && studentIndex < students.length) {
      ...
      for (
        let i = 0;
        i < coreMemberCount && studentIndex < students.length;
        i++
      ) {
        const student = students[studentIndex++];
        positionHoldersToCreate.push({
...
const seedUserSkills = async () => {
  ...
  if (userSkillsToCreate.length > 0) {
    ...
    console.log(
      `Assigned ${userSkillsToCreate.length} skills across ${students.length} students.`,
    );
  }
...
const seedEvents = async () => {
  ...
  for (const club of testClubs) {
    const eventCategory =
      club.category === "scitech" ? "technical" : club.category;

    // --- Completed Event ---
    const completedEventParticipants = [...students]
      .sort(() => 0.5 - Math.random())
      .slice(0, 5);
    if (completedEventParticipants.length === 0) {
      continue;
    }
    const completedEvent = {
      ...
      winners: [
        { user: completedEventParticipants[0]._id, position: "1st Place" },
      ],
    };
...
const seedAchievements = async () => {
  ...
  if (completedEventsWithWinners.length === 0) {
    console.log(
      "No completed events with winners found to create achievements from.",
    );
    return;
  }
  ...
  if (achievementsToCreate.length > 0) {
    ...
    console.log(
      `Created ${achievementsToCreate.length} achievements for event winners.`,
    );
  }
};
