...
const getAnnouncements = async (req, res) => {
  try {
    ...
    const query = Announcement.find(filter)
      ...
      .populate(
        "author",
        "username personal_info.email personal_info.name"
      );
  ...
  try {
    ...
    if (!isAuthor) {
      return res.status(403).json({
        error:
          "Forbidden: cannot edit this announcement",
      });
    }
    ...
    if (type || targetIdentifier) {
      ...
      const newIdentifier =
        ...
        (announcement.target_id
          ? announcement.target_id.toString()
          : null);
...
const deleteAnnouncement = async (req, res) => {
  try {
    ...
    if (!isAuthor) {
      return res.status(403).json({
        error:
          "Forbidden: cannot delete this announcement",
      });
    }
