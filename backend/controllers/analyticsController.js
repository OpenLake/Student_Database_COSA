  ...
  try {
    ...
    const plannedEvent =
      eventStats && eventStats[0] && eventStats[0].eventCounts
        ...
        : null;
    const orgUnitTypes =
      orgStats && orgStats[0] && orgStats[0].unitTypes
        ? orgStats[0].unitTypes
        : [];
    const orgBudget =
      orgStats &&
      orgStats[0] &&
      orgStats[0].budgetOverview &&
    ...
    const dashboardData = {
      summary: {
        userAnalytics: {
          totalUsers:
            (userStats && userStats[0] && userStats[0].totalUsers) || 0,
          userStatus:
            (userStats && userStats[0] && userStats[0].statusBreakdown) || [],
        ...
        eventAnalytics: {
          ...
          eventCategories:
            (eventStats && eventStats[0] && eventStats[0].eventCategories) ||
            [],
        },
        organizationalUnitAnalytics: {
          ...
          unitCategories:
            (orgStats && orgStats[0] && orgStats[0].unitCategories) || [],
        },
        porAnalytics: {
    ...
    res
      .status(500)
      .json({ message: "Server error while fetching president analytics" });
  }
};
...
exports.getClubCoordinatorAnalytics = async (req, res) => {
  try {
    ...
    if (!user || !user.personal_info || !user.personal_info.email) {
      return res
        .status(404)
        .json({ message: "User not found or email is missing." });
    }
    ...
    if (!unit) {
      return res.status(403).json({
        message:
          "Your email is not registered as the contact for any organizational unit.",
      });
    }
    ...
    res.json({
      ...
      atAGlance: {
        budget: {
          ...
          remaining:
            unitData.budget_info.allocated_budget -
            unitData.budget_info.spent_amount,
        },
        coreStats: {
          allTime: {
      ...
      teamManagement: {
        ...
        openPositions:
          (openPositions && openPositions[0] && openPositions[0].totalOpen) ||
          0,
      },
    });
  } catch (error) {
...
exports.getGensecAnalytics = async (req, res) => {
  try {
    ...
    if (
      !category ||
      !["cultural", "scitech", "sports", "academic"].includes(category)
    ) {
    ...
    const unitsInCategory = await OrganizationalUnit.find(
      { category: category },
      "_id",
    ).lean();
    ...
    if (unitIds.length === 0) {
      return res.json({
        domain: category,
        message: `No organizational units found for category: ${category}`,
      });
    }
    ...
    const [
      ...
      Promise.all([
        ...
        Event.countDocuments({
          // Total Events (Current Tenure)
          organizing_unit_id: { $in: unitIds },
          status: "completed",
          "schedule.end": { $gte: startDate, $lte: endDate },
        }),
        Event.aggregate([
          // Total Participation (Current Tenure)
          {
            $match: {
              organizing_unit_id: { $in: unitIds },
              status: "completed",
      ...
      OrganizationalUnit.aggregate([
        { $match: { category: category } },
        {
          $facet: {
            ...
            clubWise: [{ $project: { name: 1, budget: "$budget_info" } }],
          },
        },
      ]),
    ...
    res.json({
      ...
      atAGlance: {
        ...
        totalParticipation:
          (domainStats[2] && domainStats[2][0] && domainStats[2][0].total) || 0,
      },
      budgetManagement: {
        overall: budgetStats[0].overall[0] || {
          totalAllocated: 0,
          totalSpent: 0,
        },
