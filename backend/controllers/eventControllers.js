...
exports.getOrganizationalUnits = async (req, res) => {
  try {
    ...
    if (role === ROLES.PRESIDENT) {
      ...
      const coordUnit =
        await OrganizationalUnit.findOne({
...
exports.createEvent = async (req, res) => {
  try {
    ...
    const orgUnit =
      await OrganizationalUnit.findById(
        organizing_unit_id
      );

    if (!orgUnit) {
      return res.status(400).json({
        message: "Invalid organizational unit.",
      });
    }
    ...
    if (organizers && organizers.length > 0) {
      ...
      if (validUsers.length !== organizers.length) {
        return res.status(400).json({
          message:
            "One or more organizers are invalid.",
        });
      }
    }
    ...
    switch (userRole) {
      ...
      case "CLUB_COORDINATOR": {
        ...
        if (!username) {
          return res.status(400).json({
            message:
              "Username is required for Club Coordinator.",
          });
        }
        ...
        if (!orgUnit) {
          return res.status(404).json({
            message:
              "No organizational unit found for this coordinator.",
          });
        }

        query = {
          organizing_unit_id: orgUnit._id,
        };
...
exports.createRoomRequest = async (
  req,
  res
) => {
  try {
    ...
    if (!date || !time || !room) {
      return res.status(400).json({
        message:
          "Date, time, and room are required fields.",
      });
    }
    ...
    return res.status(500).json({
      message:
        "Server error while adding room request.",
    });
  }
};


exports.updateRoomRequestStatus =
  async (req, res) => {
    const { requestId } = req.params;

    const { status, reviewed_by } =
      req.body;
    ...
    if (
      ...
      return res.status(400).json({
        message:
          'A valid status ("Approved" or "Rejected") is required.',
      });
    }
    ...
    try {
      ...
      if (!event) {
        return res.status(404).json({
          message:
            "Request or associated event not found.",
        });
      }

      const request =
        event.room_requests.id(
          requestId
        );
      ...
      console.error(
        `Error updating request status to ${status}:`,
        error
      );
